...
async function uploadWithFallback(
  ...
  try {
    ...
    try {
      ...
      console.error('[PhotoUploader] ❌ Upload failed attempt', attempt, code);
      updateProgress?.(0);
      if (attempt < 2) {
        try {
          const { auth } = getFirebase();
        ...
        console.log('[PhotoUploader] Retrying upload...');
        throw await uploadWithFallback(basePath, input, updateProgress, resizeOpts, attempt + 1);
      }
      throw err;
    }
  } catch (error: any) {
...
export function PhotoUploader({
  ...
  const offlineQueueKey = useMemo(() => `photoUploaderQueue:${entityType}:${entityId}`, [entityType, entityId]);
  ...
  useEffect(() => {
    ...
    const bootstrap = async () => {
      try {
        ...
        const parsed = raw ? (JSON.parse(raw) as AnyImage[]) : [];
        offlineQueueRef.current = Array.isArray(parsed) ? parsed : [];
        if (offlineQueueRef.current.length > 0) {
          console.log('[PhotoUploader] Restored offline queue with', offlineQueueRef.current.length, 'items');
        }
      } catch {}
    };
    void bootstrap();
  ...
  const offlineQueueRef = useRef<AnyImage[]>([]);
