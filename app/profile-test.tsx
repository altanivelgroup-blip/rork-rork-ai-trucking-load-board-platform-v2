...
export default function ProfileTestScreen() {
  ...
  const showAlert = useCallback((title: string, message: string) => {
    try {
      ...
      console.error('[ProfileTest] Alert failed', e);
    }
  }, []);

  const fetchProfile = useCallback(async () => {
    console.log('[ProfileTest] Fetching profile...');
    setLoading(true);
    setError(null);
    setProfile(null);
    try {
      ...
      if (!user || !user.uid) {
        const msg = 'No signed-in user. Please log in first.';
        setUid(null);
        setError(msg);
        showAlert('Not signed in', msg);
        return;
      }
      setUid(user.uid);
      ...
      if (!snap.exists()) {
        ...
        setError(msg);
        showAlert('Missing Profile', msg);
        return;
      }

      const data = snap.data() as FirestoreDoc;
      setProfile(data);
      console.log('[ProfileTest] Profile fetched:', data);
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Unknown error fetching profile';
      console.error('[ProfileTest] Fetch error:', err);
      setError(message);
      showAlert('Fetch Error', message);
    } finally {
      setLoading(false);
    }
  }, [showAlert]);
  ...
  return (
    <View style={styles.container} testID="profileTestScreen">
      <Text style={styles.title} testID="profileTestTitle">Profile Test</Text>
