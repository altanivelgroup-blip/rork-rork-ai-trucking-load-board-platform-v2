...
export default function RunSanityCheckScreen() {
  ...
  const updateCheck = (id: string, update: Partial<SanityCheck>) => {
    setChecks(prev => prev.map(check => check.id === id ? { ...check, ...update } : check));
  };

  const generateReport = (allChecks: SanityCheck[]) => {
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      <Stack.Screen options={{ title: 'Sanity Check' }} />
